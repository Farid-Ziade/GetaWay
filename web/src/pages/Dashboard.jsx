...
export default function Dashboard() {
  ...
  async function handleGenerate() {
    ...
    try {
                ...
                t.plan?.days?.flatMap(
                  (d) =>
                    d.activities?.map((a) => {
                      ...
                      return raw
                        .replace(
                          /^(lunch|dinner|breakfast|brunch|supper|visit|explore|discover|morning|evening|afternoon|night)\s+(at|to|the|a|an)?\s*/i,
                          "",
                        )
                        .trim();
                    }) || [],
      ...
      setPlanError(
        err.message || "Could not generate your plan. Please try again.",
      );
      setPlanStatus("error");
    }
  }
  ...
  function openDayInMaps(day) {
    ...
    const stops = activities.map((act) => {
      ...
      const raw = isRevisit
        ? act.title.slice("Revisiting: ".length)
        : act.title;
  ...
  async function handleTitleSave(newTitle) {
    ...
    if (savedTripId) {
      setSavedTrips((prev) =>
        prev.map((t) => (t.id === savedTripId ? { ...t, title: trimmed } : t)),
      );
      try {
        ...
        setSavedTrips((prev) =>
          prev.map((t) =>
            t.id === savedTripId ? { ...t, title: prevTitle } : t,
          ),
        );
      }
    }
          ...
          {tripsLoading ? (
            ...
            <div className={styles.savedEmpty}>
              ...
              <p>
                No saved trips yet.
                <br />
                Generate a plan and save it here.
              </p>
            </div>
          ) : (
              ...
              {savedTrips.map((t) => (
                ...
                >
                  ...
                  <span className={styles.savedItemDate}>
                    {t.savedAt.toLocaleDateString("en-US", {
                      month: "short",
                      day: "numeric",
                    })}
                    {" at "}
                    {t.savedAt.toLocaleTimeString("en-US", {
                      hour: "numeric",
                      minute: "2-digit",
                    })}
                  </span>
      ...
      <main className={styles.right}>
        {planStatus === "idle" && (
          <div className={styles.emptyState}>
            ...
            <p className={styles.emptyDesc}>
              Set your location and budget on the left,
              <br />
              then hit <strong>Generate My Weekend</strong>.
            </p>
          </div>
        )}
        ...
        {planStatus === "loading" && (
          <div className={styles.emptyState}>
            ...
            <p className={styles.emptyDesc}>
              Checking the weather, finding nearby spots, calculating your
              budget.
            </p>
          </div>
        )}
            ...
            <div className={styles.planHeader}>
              <div>
                {isEditingTitle ? (
                  ...
                  <div
                    ...
                    onClick={() => {
                      ...
                      setEditTitleVal(planTitle || plan.title);
                    }}
                  >
                ...
                <div className={styles.planMeta}>
                  {plan.weather && (
                    <span>
                      <WeatherIcon /> {plan.weather}
                    </span>
                  )}
                  {plan.totalCost && (
                    <span>
                      <CostIcon /> Est. {plan.totalCost} total
                    </span>
                  )}
                </div>
              </div>
              ...
              <div key={di} className={styles.daySection}>
                ...
                <div className={styles.timeline}>
                  {day.activities.map((act, ai) => {
                    const isRevisit = act.title?.startsWith("Revisiting: ");
                    const displayTitle = isRevisit
                      ? act.title.slice("Revisiting: ".length)
                      : act.title;
                    return (
                      <div
                        key={ai}
                        className={`${styles.activity} ${styles[`type_${act.type}`]}`}
                      >
                        ...
                        <div className={styles.activityBody}>
                          <div className={styles.activityTop}>
                            <div className={styles.activityTitleRow}>
                              ...
                              {isRevisit && (
                                <span className={styles.revisitBadge}>
                                  Revisiting
                                </span>
                              )}
                            </div>
                              ...
                              <button
                                ...
                                onClick={() => {
                                  const country = locationLabel.includes(",")
                                    ...
                                    : locationLabel;
                                  const clean =
                                    stripActivityPrefix(displayTitle);
                                  const label = country
                                    ? `${clean}, ${country}`
                                    : clean;
                                  if (act.placeId) {
...
function SpinnerIcon() {
  return (
    <svg
      ...
      strokeLinecap="round"
    >
      <path d="M21 12a9 9 0 1 1-6.219-8.56" className="spin" />
    </svg>
  );
}
